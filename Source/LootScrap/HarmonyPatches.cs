...
namespace LootScrap
{
    ...
    public static class HarmonyPatches
    {
        ...
        static HarmonyPatches()
        {
            ...
            // Patch Corpse.SpawnSetup to handle pre-existing corpses found on map
            var corpseSpawnMethod = AccessTools.Method(typeof(Corpse), "SpawnSetup");
            if (corpseSpawnMethod != null)
            {
                harmony.Patch(
                    corpseSpawnMethod,
                    postfix: new HarmonyMethod(typeof(HarmonyPatches).GetMethod(nameof(CorpseSpawn_Postfix)))
                );
            }
            else
            {
                Log.Error("[LootScrap] Failed to find Corpse.SpawnSetup!");
            }
        ...
        public static void CorpseSpawn_Postfix(Corpse __instance)
        {
            try
            {
